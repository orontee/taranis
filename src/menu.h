...
public:
  ...
  MenuButton(int pos_x, int pos_y, std::shared_ptr<Icons> icons)
      : activated{false},
        items{imenu{ITEM_ACTIVE, taranis::MENU_ITEM_REFRESH,
              ...
              imenu{ITEM_INACTIVE, taranis::MENU_ITEM_ABOUT,
  ...
  void show() {
    ...
    const int icon_pos_x = this->bounding_box.x + (MenuButton::width - MenuButton::icon_size) / 2;
    const int icon_pos_y = this->bounding_box.y + (MenuButton::height - MenuButton::icon_size) / 2;
